...
router.get('/list/:page/:type/:keywords',(req,res)=>{
    ...
    switch(type){
        ...
        case '3':type='sponsored_broker';break;
        default:type='code';break;
    }

    query("select code,company,owner,legal_representative,profile,registered_capital,sponsored_broker" +
        " from company_info where "+type+" like ?",[`%${keywords}%`],(err,data)=>{
...
router.get('/geo/:type',(req,res)=>{
    ...
    else addr='office_addr';
    query("select "+addr+" as addr from company_info",[],(err,data)=>{
        if(err) return res.json({
            code:-1,
            msg:'获取公司地址分布失败'
        })
